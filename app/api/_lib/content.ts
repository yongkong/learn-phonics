// 静态教学内容：音素表 + CVC 词库（无需数据库，直接随函数打包）
// 顺序依据 Letters and Sounds (DfE 2007) Phase 2 Set 1-5 + Phase 3 字母集
type LetterSeed = [letter: string, sound: string, say: string, words: string[], phase: number, setNo: number | null]
const LETTER_SEED: LetterSeed[] = [
...
export const PHONEMES: PhonemeRecord[] = LETTER_SEED.map((s, i) => ({
  letter: s[0], ord: i, sound: s[1], say: s[2], words: s[3], phase: s[4], set_no: s[5],
}))

export const WORDS: WordRecord[] = WORD_SEED.map(([word, letters, meaning, level]) => ({
  word, letters, says: letters.map((l) => SAYS[l] ?? l), meaning, level,
}))
